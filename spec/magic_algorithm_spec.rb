...
require_relative '../app/magic_algorithm.rb'
...
RSpec.describe MagicAlgorithm do
  ...
  xit 'returns a list with 4 cache_server' do
    expect(MagicAlgorithm.new(@world).solve.cache_servers.count).to eq(3)
  end

  xit 'returns cache_server 0' do
    expect(MagicAlgorithm.new(@world).solve.cache_servers[0].videos).to eq([@videos[1], @videos[3]])
  end

  xit 'returns cache_server 1 & 2 to be empty' do
    expect(MagicAlgorithm.new(@world).solve.cache_servers[1].videos).to eq([])
    expect(MagicAlgorithm.new(@world).solve.cache_servers[2].videos).to eq([])
  end
end
